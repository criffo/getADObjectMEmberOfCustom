function Get-ADObjectMemberOfCustom {
    <#
        ...
        .PARAMETER Scope
        Specifies the search scope. Validateset. Not Mandatory. Default is Forest.
        ...
        .NOTES
            ...
            In order to work, the machine from which the function is executed needs TCP 389 opened to each queried domains

            In a nuthshell, the function based on the parameters will test if the ad object exists
            builds a hashtable to contain all domains in scope
            Assesses if domain users or computers has to be included
            will put in a hashtable all collected groups with the distinguished name paired as a key
            the principle is a breadth first search
            we always check in the hashtable of collected items if already collected to avoid any unuseful query
            and quick access to information
            in order to avoid infinite loop because of crossed nesting between 2 groups, we limit to the first found level
            each memberof attribute is assessed and in caseof  based on dn the query is not done on same forest 
            as the initial ad object, object sid is converted to sid and fsp DN string is built to search in the external domain
            the loop while will enrich 2 memberof lists alternatively regarding the nesting level that is incremented each
            time a list is emptied and will proceed the other list
            exit of the loop when no item in both lists for the BFS
            In case of domain local group, no search is performed outside the domain that they belong to
            the items from SearchResultCollection of all directory searcher queries are 
            pointed from a hastable paired with dn of the concerned object
            we dispose the directory searcher
            We use the enumeration on this hashtable and format new object to priduce the results in a list of object 
            with string attributes

            In case you need to access each item of memberof attribute of a group
            proceed a split method on carriage return tehn access by index or enumerator.
    #>
    param(
                ...
                if ($DomainToQuery."$DomTargetMain" -eq $DomainToQuery."$DomRefofCurrentObjectToControlMemberOf") {
                    ...
                    if ($resultTemp -ne $null) {
                        foreach ($item in $resultTemp) {
                            ...
                            $item.domain = $DomRefofCurrentGroup
                            $item.memberof = $memberOf
                            if (-not ($ObjectAlreadyQueried."$dn")) {$ObjectAlreadyQueried.add([string]$item.distinguishedname.Trim(),$item)}                           
                            $memberOfTemp.Clear()
                        }
                    }
                }
