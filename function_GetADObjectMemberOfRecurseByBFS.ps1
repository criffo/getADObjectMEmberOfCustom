function Get-ADObjectMemberOfRecurseByBFS {
    <#
        ...
        .PARAMETER IncludePrimaryGroup
        Specifies to include the primary group in the search. Default $false, not mandatory.
        ...
        .INPUTS
        None. You cannot pipe objects to Get-ADObjectMemberOfRecurseByBFS.

        .OUTPUTS
        System.Collections.Generic.List[object]. Get-ADObjectMemberOfRecurseByBFS a list of objects with string attributes in order to allow export-csv
        Nothing is returned in case of no result.

        .EXAMPLE
        C:\PS> Get-ADObjectMemberOfRecurseByBFS samcountname global.contoso.com

        .EXAMPLE
        C:\PS> Get-ADObjectMemberOfRecurseByBFS distinguishedname global.contoso.com $true ForestAndDomainTrusts -SearcherPageSize 1000

        .EXAMPLE
        C:\PS> Get-ADObjectMemberOfRecurseByBFS distinguishedname global.contoso.com $true ExplicitDomains "corp1.constoso.com","corp2.contoso.com","external.com"
    ...
    if ($InitialADObjectResult -eq $null) {
        ...
        #as we don t find the primary group in the direct memberof we can incliude the switch to search as well that group
        ...
        #we keep  values that are needed further from object $InitialADObjectResult
        $QueriedObjSID = $InitialADObjectResult.properties.objectsid
        $QueriedObjMemberOfListDN = New-Object System.Collections.Generic.List[string]
        if ($InitialADObjectResult.properties.memberof) {
            $InitialADObjectResult.properties.memberof | %{
                $QueriedObjMemberOfListDN.Add($psitem)
            }
        }
        $QueriedObjDN = $InitialADObjectResult.properties.distinguishedname
        if ($IncludePrimaryGroup -eq $true) {
            $stringSID = New-Object System.Security.Principal.SecurityIdentifier($InitialADObjectResult.properties.objectsid[0], 0)
            $primaryGroupSIDString = [string]$stringSID.AccountDomainSid + "-" + [string]$InitialADObjectResult.properties.primarygroupid
            $Searcher.Filter = "(objectSid=" + $primaryGroupSIDString + ")"
            $resultTemp = $Searcher.FindOne()
            if ($resultTemp -ne $null) { $QueriedObjMemberOfListDN.Add($resultTemp.properties.distinguishedname)}
        }
        if ($QueriedObjMemberOfListDN) {
            $QueriedObjMemberOfListDN | %{
        ...
        $DomainToQuery.keys.ForEach({
            ...
            if ($DomTargetMain -ne $DomRefofCurrentObjectToControlMemberOf) {
                if ($DomainToQuery."$DomTargetMain" -eq $DomainToQuery."$DomRefofCurrentObjectToControlMemberOf") {
                    $Searcher.Filter = "(member=" + $QueriedObjDN + ")"
                    ...
                    $keySID = (New-Object System.Security.Principal.SecurityIdentifier($QueriedObjSID[0], 0)).toString()
